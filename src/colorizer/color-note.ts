...
const isOddNote = (note: Note) => 
    ["C", "E", "G", "H"].includes(note.sign)

const isOddOctate = (node: Note) =>
    !!(node.octave % 2)
...
const calculatePosition = (note: Note): Position => {
    ...
    const depth = (note.octave - 4) * 4 + Math.ceil(signValues.indexOf(note.sign) / 2);
    return { side, depth };
}
...
export const colorNote = (note: Note): string => {
    ...
    const color = position.side === 'left' ? `hsl(264, 60%, ${l}%)` : `hsl(130, 50%, ${l}%)`;
    console.log(color);
    return color;
}